...
namespace Nsia.Controllers
{
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> Dashboard()
        {
            ...
            ViewBag.Draft = apps.Count(a => a.Status == "Draft");
            ViewBag.Verified = apps.Count(a => a.IsEmailVerified);
            ViewBag.Unverified = apps.Count(a => !a.IsEmailVerified);

            // Step completion breakdown
            ViewBag.StepBreakdown = Enumerable.Range(1, 7)
                .Select(s => new { Step = s, Count = apps.Count(a => a.ApplicationStep >= s) })
                .ToList();

            // Sector breakdown
            ViewBag.SectorBreakdown = apps
                .Where(a => !string.IsNullOrEmpty(a.Sector))
                .GroupBy(a => a.Sector)
            ...
            ViewBag.StageBreakdown = apps
                .Where(a => !string.IsNullOrEmpty(a.GrowthStage))
                .GroupBy(a => a.GrowthStage)
            ...
            ViewBag.DailyRegistrations = apps
            ...
            ViewBag.RecentApplications = apps
